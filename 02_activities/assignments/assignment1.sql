...
SELECT vendor_id,count(vendor_id) as count_of_renting_booths
...
SELECT C.customer_id, c.customer_first_name,c.customer_last_name,sum(cp.quantity*cp.cost_to_customer_per_qty) as total_cost_spent
FROM customer c
...
FROM
    customer_purchases
WHERE
    strftime('%Y', market_date) = '2022'  -- Filter for the year 2022
    AND strftime('%m', market_date) = '04'  -- Filter for the month of April
GROUP BY
    customer_id;